...
export function renderSensitive(state: TimelineItemState, handlers: TimelineItemHandlers) {
    return html`
    <div class="sensitive">
...
export function renderReplyContext(state: TimelineItemState, handlers: TimelineItemHandlers) {
...
export function renderRegularTweet(state: TimelineItemState, handlers: TimelineItemHandlers) {
    return html`
    ...
    >
      <div class="header-actions-block" slot="header">
        ...
        <div class="actions-right">
          <md-icon-button
            size="small"
            @click="${() => handlers.translatePost(state.tweet?.content || null)}"
          >
        ...
        <!-- <span>
          ${new Intl.RelativeTimeFormat('en', {
                ...
                Math.floor(
                    -(
                        (new Date() as any) -
                        (new Date(state.tweet?.created_at || '') as any)
                    ) /
                    1000 /
                    60
                ),
      ...
      ${state.tweet && state.tweet.card
            ? html`
            <div
              @click="${() => handlers.openLinkCard(state.tweet?.card?.url || '')}"
              class="link-card"
            >
              <img
...
export function renderReblog(state: TimelineItemState, handlers: TimelineItemHandlers) {
...
export function renderThread(state: TimelineItemState, handlers: TimelineItemHandlers) {
